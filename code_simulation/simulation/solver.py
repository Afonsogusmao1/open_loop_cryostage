...
from code_simulation.simulation.geometry import (
    GeometryParams,
...
from code_simulation.simulation.front_tracking import prepare_point_eval, eval_prepared, front_from_temperature_threshold
...
class PrefillOptions:
    mode: str = "steady"  # "steady", "transient", "fixed_time", "probe_stabilized", "water_probe_stabilized"
    ...
    *,
    ...
    if fill_flag < 0.5:
        ...
        if use_tabulated_water_ice:
            k_cav, cp_cav, rho_cav = water_ice_k_cp_rho_from_tables(T_cav, phase.Tf)
        else:
            k_cav = np.full(T_cav.shape, water_const.k, dtype=np.float64)
            cp_cav = np.full(T_cav.shape, water_const.cp, dtype=np.float64)
            rho_cav = np.full(T_cav.shape, water_const.rho, dtype=np.float64)

        cp_eff_cav = cp_cav + apparent_cp_bump(T_cav, phase.Tf, phase.L_latent, phase.dT_mushy)

        k_arr[fillable_dof] = k_cav
        rho_cp_arr[fillable_dof] = rho_cav * cp_eff_cav
...
def run_case(
    *,
    ...
    T_plate_profile_time_origin: str = "absolute",
    ambient_temperature_from_plate_C: Callable[[float], float] | None = None,
    ...
    Supported pre-fill modes:
      ...
      - "water_probe_stabilized": fill water at t=0, hold the plate warm until probes stabilize,
        then start the supplied plate-temperature profile
    ...
    ambient_temperature_from_plate_C optionally maps the current modeled plate
    temperature to the convective ambient temperature. When omitted, the solver
    keeps using bcs.T_room_C as the fixed ambient value.

    T_plate_profile_time_origin controls how T_plate_profile_C is sampled. Use
    "absolute" for existing workflows. Use "after_control_start" with
    water_probe_stabilized so the profile clock starts after the warm-water hold.
    """
    comm = MPI.COMM_WORLD
    ...
    profile_time_origin = str(T_plate_profile_time_origin).strip().lower()
    if profile_time_origin not in {"absolute", "after_control_start"}:
        raise ValueError("T_plate_profile_time_origin must be 'absolute' or 'after_control_start'")
    if profile_time_origin == "after_control_start" and prefill_mode not in {
        "water_probe_stabilized",
        "water_fixed_time",
    }:
        raise ValueError(
            "T_plate_profile_time_origin='after_control_start' requires a water-filled warm-hold prefill mode"
        )

    cooling_start_time: float | None = 0.0 if profile_time_origin == "absolute" else None
    cooling_start_reason = "absolute_profile_origin" if profile_time_origin == "absolute" else ""

    def warm_hold_temperature_C() -> float:
        if prefill.hold_temperature_C is None:
            return float(T_plate_C)
        return float(prefill.hold_temperature_C)

    def plate_temperature_C(current_t: float) -> float:
        current_t = float(current_t)
        profile_t = current_t
        if profile_time_origin == "after_control_start":
            if cooling_start_time is None or current_t < cooling_start_time:
                return warm_hold_temperature_C()
            profile_t = max(current_t - cooling_start_time, 0.0)
        if T_plate_profile_C is None:
            return float(T_plate_C)
        return float(T_plate_profile_C(float(profile_t)))

    def ambient_temperature_C(current_t: float) -> float:
        if ambient_temperature_from_plate_C is None:
            return float(bcs.T_room_C)
        return float(ambient_temperature_from_plate_C(plate_temperature_C(current_t)))
    ...
    elif prefill_mode == "water_probe_stabilized":
        t_prefill_budget = max(0.0, prefill.max_prefill_s)
    elif prefill_mode == "water_fixed_time":
        t_prefill_budget = max(0.0, pre_cool_s)
    else:
    ...
    with xdmf_context as xdmf:
        ...
        def start_cooling_profile(current_t: float, *, reason: str) -> None:
            nonlocal cooling_start_time, cooling_start_reason
            if cooling_start_time is not None:
                return
            cooling_start_time = float(current_t)
            cooling_start_reason = str(reason)
            if show_progress and comm.rank == 0:
                print(f"\nCooling trajectory starts at t={current_t:.3f}s ({reason}).")

        if prefill_mode == "steady" and pre_cool_s <= 1e-12:
            do_fill(0.0)
        elif prefill_mode in {"water_probe_stabilized", "water_fixed_time"}:
            do_fill(0.0)
        ...
        if float(fill.value) < 0.5 or (
            prefill_mode == "water_probe_stabilized" and cooling_start_time is None
        ):
        ...
        for step in range(1, n_steps + 1):
            ...
            if float(fill.value) < 0.5:
                if prefill_mode in {"transient", "fixed_time"} and (t >= pre_cool_s - 1e-12):
                    do_fill(t)
            elif prefill_mode == "water_fixed_time" and cooling_start_time is None and (t >= pre_cool_s - 1e-12):
                start_cooling_profile(t, reason="fixed_water_hold_time")
            ...
            if (
                ...
                if due_debug:
                    ...
                    if use_tabulated_water_ice:
                        k_tab, cp_tab, rho_tab = water_ice_k_cp_rho_from_tables(np.array([T_dbg]), phase.Tf)
                        k_tab = float(k_tab[0])
                        cp_tab = float(cp_tab[0])
                        rho_used = float(rho_tab[0])
                    else:
                        k_tab = float(water_const.k)
                        cp_tab = float(water_const.cp)
                        rho_used = float(water_const.rho)

                    cp_eff_dbg = float(rho_cp_used[idx_dbg] / rho_used)
            ...
            Troom_c.value = PETSc.ScalarType(ambient_temperature_C(t))
            problem.solve()
            T_prev.x.array[:] = T.x.array
            T_prev.x.scatter_forward()
                ...
                if prefill_mode == "probe_stabilized":
                    ...
                    if t >= prefill.min_prefill_s and _probe_stabilized(
                        ...
                        if show_progress and comm.rank == 0:
                            spans = np.ptp(np.asarray(probe_values, dtype=np.float64), axis=0)
                            print(
                                "\nWATER-FILLED probes stabilized -> "
                                f"spans over last {prefill.probe_window_s:.1f}s = "
                                f"[{spans[0]:.4f}, {spans[1]:.4f}, {spans[2]:.4f}] °C"
                            )
                        start_cooling_profile(t, reason="water_probe_stabilized")
                    elif t >= prefill.max_prefill_s - 1e-12:
                        raise RuntimeError(
                            "Water-filled probe stabilization criterion was not reached before max_prefill_s. "
                            "Increase max_prefill_s or relax probe_window_s / probe_tol_C."
                        )
            ...
            if (
                profile_time_origin == "after_control_start"
                and cooling_start_time is not None
                and t - cooling_start_time >= t_after_fill_s - 1e-12
            ):
                if show_progress and comm.rank == 0:
                    print(
                        f"\nReached requested post-cooling horizon "
                        f"t_since_cooling_start={t - cooling_start_time:.2f}s. Stopping."
                    )
                if xdmf is not None:
                    xdmf.write_function(T, t)
                break
        ...
        if comm.rank == 0:
            ...
            with timing_path.open("w", newline="") as f_timing:
                f_timing.write("parameter,value\n")
                f_timing.write(f"prefill_mode,{prefill_mode}\n")
                f_timing.write(f"profile_time_origin,{profile_time_origin}\n")
                f_timing.write(f"fill_time_s,{fill_time if fill_time is not None else 'nan'}\n")
                f_timing.write(
                    f"cooling_start_time_s,{cooling_start_time if cooling_start_time is not None else 'nan'}\n"
                )
                f_timing.write(f"cooling_start_reason,{cooling_start_reason}\n")
                f_timing.write(f"warm_hold_temperature_C,{warm_hold_temperature_C():.9f}\n")
                f_timing.write(f"requested_post_cooling_horizon_s,{float(t_after_fill_s):.9f}\n")
