...
RHO_TABLE_ICE = np.array(
    [925.7, 924.9, 924.1, 923.3, 922.4,
     921.6, 920.8, 920.4, 920.0, 919.6,
     919.4, 919.4, 918.9, 917.5, 916.2],
    dtype=np.float64
)
...
def water_ice_k_cp_rho_from_tables(T_C: np.ndarray, Tf: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    k(T), cp(T), rho(T) by table lookup with phase selection:
...
def water_ice_k_cp_from_tables(T_C: np.ndarray, Tf: float) -> tuple[np.ndarray, np.ndarray]:
    """
    Backward-compatible wrapper returning only k(T) and cp(T).
    """
    k, cp, _rho = water_ice_k_cp_rho_from_tables(T_C, Tf)
    return k, cp
