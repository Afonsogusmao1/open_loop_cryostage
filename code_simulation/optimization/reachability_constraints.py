...
from code_simulation.core.arrays import coerce_float_tuple as _as_float_tuple
from code_simulation.core.paths import CODE_SIMULATION_ROOT, PROJECT_ROOT
...
def _coerce_constraints_dir(path: str | Path | None) -> Path:
    ...
    search_roots.extend((Path.cwd(), CODE_SIMULATION_ROOT, PROJECT_ROOT))
...
def _conservative_bracketing_value(
    ...
    *,
    ...
    temperature_margin_C = float(temperature_margin_C)
    if not math.isfinite(temperature_margin_C) or temperature_margin_C < 0.0:
        raise ValueError("temperature_margin_C must be finite and non-negative")
    min_target_C, max_target_C = constraints.supported_cooling_target_range_C
    if (
        target_temperature_C < min_target_C - temperature_margin_C - 1e-12
        or target_temperature_C > max_target_C + temperature_margin_C + 1e-12
    ):
        return math.nan
    ...
    close_idx = np.flatnonzero(np.abs(targets - target_temperature_C) <= temperature_margin_C + 1.0e-12)
    if close_idx.size:
        best_close_idx = int(close_idx[np.argmin(np.abs(targets[close_idx] - target_temperature_C))])
        return float(values[best_close_idx])
    exact_idx = np.flatnonzero(np.isclose(targets, target_temperature_C, atol=1e-12))
    if exact_idx.size:
        return float(values[int(exact_idx[0])])

    if target_temperature_C < float(targets[0]):
        return float(values[0])
    if target_temperature_C > float(targets[-1]):
...
def conservative_first_entry_time_s(target_temperature_C: float, constraints: ReachabilityConstraints) -> float:
    return _conservative_bracketing_value(
        target_temperature_C,
        constraints.first_entry_time_s_by_target_C,
        constraints,
    )
...
def conservative_settling_time_s(target_temperature_C: float, constraints: ReachabilityConstraints) -> float:
    return _conservative_bracketing_value(
        target_temperature_C,
        constraints.settling_time_s_by_target_C,
        constraints,
    )


def direct_characterization_support_band_C(
    constraints: ReachabilityConstraints,
    *,
    temperature_margin_C: float = 0.0,
) -> tuple[float, float]:
    ...
    min_target_C, max_target_C = constraints.supported_cooling_target_range_C
    return (
        float(min_target_C - temperature_margin_C),
        float(max_target_C + temperature_margin_C),
    )


def is_within_direct_characterization_support(
    target_temperature_C: float,
    constraints: ReachabilityConstraints,
    *,
    temperature_margin_C: float = 0.0,
) -> bool:
    lower_C, upper_C = direct_characterization_support_band_C(
        constraints,
        temperature_margin_C=temperature_margin_C,
    )
    return lower_C - 1e-12 <= float(target_temperature_C) <= upper_C + 1e-12
...
def conservative_hold_support_s(
    ...
    *,
    ...
    if (
        target_temperature_C < min_hold_target_C - temperature_margin_C - 1e-12
        or target_temperature_C > max_transient_target_C + temperature_margin_C + 1e-12
    ):
        return math.nan, math.nan

    hold_targets = np.asarray(constraints.observed_hold_targets_C, dtype=np.float64)
    close_idx = np.flatnonzero(np.abs(hold_targets - target_temperature_C) <= temperature_margin_C + 1.0e-12)
    if close_idx.size:
        basis_idx = int(close_idx[np.argmin(np.abs(hold_targets[close_idx] - target_temperature_C))])
    ...
    if target_temperature_C < float(hold_targets[0]):
        basis_idx = 0
    elif target_temperature_C > float(hold_targets[-1]):
        basis_idx = hold_targets.size - 1
    else:
        basis_idx = int(np.searchsorted(hold_targets, target_temperature_C, side="right") - 1)
        basis_idx = min(max(basis_idx, 0), hold_targets.size - 1)
    basis_target_C = float(hold_targets[basis_idx])
    return float(constraints.hold_supported_duration_s_by_target_C[basis_target_C]), basis_target_C
...
def check_segment_admissibility(
    *,
    ...
    constraints: ReachabilityConstraints,
    require_monotone_nonincreasing: bool,
    allowed_cold_extrapolation_C: float = 0.0,
    characterization_temperature_margin_C: float = 0.0,
    ...
    allowed_cold_extrapolation_C = float(allowed_cold_extrapolation_C)
    if not math.isfinite(allowed_cold_extrapolation_C) or allowed_cold_extrapolation_C < 0.0:
        raise ValueError("allowed_cold_extrapolation_C must be finite and non-negative")
    characterization_temperature_margin_C = float(characterization_temperature_margin_C)
    if (
        not math.isfinite(characterization_temperature_margin_C)
        or characterization_temperature_margin_C < 0.0
    ):
        raise ValueError("characterization_temperature_margin_C must be finite and non-negative")

    min_supported_target_C, warmest_characterized_target_C = constraints.supported_cooling_target_range_C
    coldest_allowed_target_C = min_supported_target_C - allowed_cold_extrapolation_C
    supported_target_range_check_passed = float(T_end_C) >= coldest_allowed_target_C - 1e-12
    if not supported_target_range_check_passed:
        reasons.append(
            f"segment endpoint {float(T_end_C):.2f} C is colder than allowed cold limit {coldest_allowed_target_C:.2f} C"
        )
    ...
    if hold_like_after_segment and is_within_direct_characterization_support(
        ...
        conservative_first_entry = _conservative_bracketing_value(
            float(T_end_C),
            constraints.first_entry_time_s_by_target_C,
            constraints,
            temperature_margin_C=characterization_temperature_margin_C,
        )
        conservative_settling = _conservative_bracketing_value(
            float(T_end_C),
            constraints.settling_time_s_by_target_C,
            constraints,
            temperature_margin_C=characterization_temperature_margin_C,
    ...
    if empirical_hold_check_applied:
        conservative_supported_hold_duration_s, empirical_hold_basis_target_C = conservative_hold_support_s(
            float(T_end_C),
            constraints,
            temperature_margin_C=characterization_temperature_margin_C,
        )
    ...
    *,
    ...
    for segment_index in range(1, len(knot_times)):
        ...
        segment = check_segment_admissibility(
            ...
            characterization_temperature_margin_C=characterization_temperature_margin_C,
        )
