...
class OpenLoopOptimizationResult:
    ...
    bayesian_backend_result: BayesianOptimizationBackendResult | None = None
...
def optimize_open_loop_theta(
    ...
    *,
    ...
    objective_evaluator: Callable[..., OpenLoopObjectiveResult] = evaluate_open_loop_objective,
    ...
    def evaluate_candidate(theta, phase: str, raw_candidate=None) -> float:
        ...
        try:
            prepared_candidate = prepare_open_loop_candidate(theta_tuple, config)
        except Exception as exc:
            error_message = f"{type(exc).__name__}: {exc}"
        else:
            expensive_simulation_executed = True
            try:
                objective_result = objective_evaluator(
                    theta_tuple,
                    config,
                    cryostage_params,
                    out_dir,
    ...
    if use_bayesian_backend:
        ...
        backend_result: BayesianOptimizationBackendResult = run_bayesian_optimization(
            ...
            precheck_candidate=lambda theta: prepare_open_loop_candidate(theta, config),
        )
        ...
        try:
            prepare_open_loop_candidate(optimizer_theta, config)
        except Exception as exc:
            success = False
            message = (
    ...
    return OpenLoopOptimizationResult(
        ...
        bayesian_backend_result=backend_result if use_bayesian_backend else None,
    )
