...
from code_simulation.core.arrays import coerce_float_tuple as _coerce_float_tuple
from code_simulation.core.arrays import validate_strictly_increasing as _validate_strictly_increasing
from code_simulation.core.config_files import SimulationProfileConfig
from code_simulation.simulation.geometry import GeometryParams
from code_simulation.optimization.reachability_constraints import default_constraints_dir
from code_simulation.simulation.solver import FreezeStopOptions, PrefillOptions, ThermalBCs, normalize_front_definition_mode
...
DEFAULT_CHARACTERIZATION_CONSTRAINTS_DIR = default_constraints_dir()
...
class OpenLoopProblemConfig:
    ...
    def __post_init__(self) -> None:
        ...
        allowed_cold_extrapolation_C = float(self.allowed_cold_extrapolation_C)
        ...
        object.__setattr__(self, "allowed_cold_extrapolation_C", allowed_cold_extrapolation_C)
        object.__setattr__(
            self,
...
def apply_simulation_profile(
    ...
    """Apply an external simulation profile without changing objective-level settings."""
    solver_kwargs = dict(config.solver_kwargs)
    solver_kwargs.update(
        {
            "Nr": int(profile.Nr),
            "Nz": int(profile.Nz),
            "dt": float(profile.solver_dt_s),
            "write_every": float(profile.write_every_s),
            "write_field_output": bool(profile.write_field_output),
            "write_probe_csv": bool(profile.write_probe_csv),
            "show_progress": bool(profile.show_progress),
            "Nz_front": int(profile.Nz_front),
            "Nr_front_curve": int(profile.Nr_front_curve),
            "Nz_front_curve": int(profile.Nz_front_curve),
            "enable_front_curve": bool(profile.enable_front_curve),
            "use_tabulated_water_ice": bool(profile.use_tabulated_water_ice),
        }
    )
    if profile.ambient_temperature_from_plate_C is None:
        solver_kwargs.pop("ambient_temperature_from_plate_C", None)
    else:
        solver_kwargs["ambient_temperature_from_plate_C"] = profile.ambient_temperature_from_plate_C

    return replace(
        config,
        cryostage_dt_s=float(profile.cryostage_dt_s),
