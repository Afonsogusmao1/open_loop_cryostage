...
if TYPE_CHECKING:
    ...
    from code_simulation.simulation.open_loop_cascade import OpenLoopCascadeResult
...
def validate_theta_reachability_admissibility(
    ...
    report = check_piecewise_linear_trajectory_admissibility(
        ...
        allowed_cold_extrapolation_C=config.allowed_cold_extrapolation_C,
...
def front_definition_contract_summary(config: OpenLoopProblemConfig) -> dict[str, float | str]:
    from code_simulation.simulation.solver import (
        FreezeStopOptions,
        PhaseChangeParams,
        front_threshold_from_mode,
        normalize_front_definition_mode,
    )
