...
DEFAULT_OUTPUT_ROOT = velocity_control_diagnostic_dir(3, "bo_monotone_feasible_admissible_range")
...
TUNED_INIT_POINTS = 2
TUNED_N_ITER = 30
ROBUST_REQUIRED_SPEED_SUCCESSES = 4
...
def _target_command(run_name: str, target_mm_s: float, schedule: str, seed: int) -> str:
    ...
    theta0_arg = ",".join(f"{float(value):.15g}" for value in TUNED_THETA0_C)
    return (
        f"cd {repo_root}\n"
...
def _load_tuned_bo_run(path: Path, *, simulation_config_path: Path, schedules: tuple[str, ...]) -> RunMetrics | None:
...
def _load_baseline_bo_run(path: Path, *, simulation_config_path: Path, schedules: tuple[str, ...]) -> RunMetrics | None:
    _ensure_bo_plate_tracking_artifacts(path, simulation_config_path=simulation_config_path, schedules=schedules)
    effective_path = path / "effective_config.toml"
    tracking_path = path / "best_tracking_summary.csv"
    theta_path = path / "best_theta_profile.csv"
    ...
    bo_cfg = dict(config.get("bayesian_optimization", {}))
    run_name = str(run_cfg.get("run_name", path.name))
    if f"_{RUN_NAME_TAG}_" in run_name:
        return None
    num_knots = int(trajectory_cfg.get("num_knots", 0))
    if num_knots != STUDY_NUM_KNOTS:
        return None
...
def _group_rows(rows: list[dict[str, object]], *, group_keys: tuple[str, ...]) -> list[dict[str, object]]:
    ...
    for key in sorted(grouped):
        ...
        robustly_recovered = bool(
            num_completed >= len(DEFAULT_SEEDS)
            and n_speed_success >= ROBUST_REQUIRED_SPEED_SUCCESSES
...
def _baseline_comparison_rows(
    *,
    tuned_aggregates: list[dict[str, object]],
    ...
    for target_mm_s in targets_mm_s:
        for schedule in schedules:
            tuned = _find_group_aggregate(tuned_aggregates, target_mm_s=target_mm_s, schedule=schedule)
            ...
            rows.append(
                {
                    ...
                    "baseline_median_direct_speed_relative_error_pct": math.nan if baseline is None else baseline["median_direct_speed_relative_error_pct"],
                    "baseline_median_infeasible_fraction": math.nan if baseline is None else baseline["median_infeasible_fraction"],
                    "baseline_late_best_fraction": math.nan if baseline is None else baseline["late_best_fraction"],
                    "baseline_achieved_speed_spread_mm_s": math.nan if baseline is None else baseline["achieved_speed_spread_mm_s"],
                    "tuned_num_completed": 0 if tuned is None else tuned["num_completed"],
                    "tuned_speed_success_fraction": math.nan if tuned is None else tuned["speed_success_fraction"],
                    "tuned_median_direct_speed_relative_error_pct": math.nan if tuned is None else tuned["median_direct_speed_relative_error_pct"],
                    "tuned_median_infeasible_fraction": math.nan if tuned is None else tuned["median_infeasible_fraction"],
                    "tuned_late_best_fraction": math.nan if tuned is None else tuned["late_best_fraction"],
                    "tuned_achieved_speed_spread_mm_s": math.nan if tuned is None else tuned["achieved_speed_spread_mm_s"],
                }
            )
    return rows
...
def _plot_target_vs_achieved_by_schedule(
    ...
    *,
    ...
    if len(schedules) == 1:
        axes = [axes]
    color_by_seed = {17: "#1f77b4", 29: "#2ca02c", 41: "#d62728", 53: "#9467bd", 67: "#8c564b"}
    completed = [
        row for row in rows
    ...
    if not completed:
        for ax in axes:
            ax.axis("off")
            ax.text(0.02, 0.95, "No completed tuned runs available yet.", va="top", ha="left")
    ...
    for ax, schedule in zip(axes, schedules, strict=True):
        ...
        ax.set_title(titles.get(schedule, schedule))
    ...
    fig.suptitle("Tuned n3 BO: target vs achieved direct speed by schedule", fontsize=14)
    fig.tight_layout()
    fig.savefig(out_path)
    plt.close(fig)
...
def _readme_text(*, targets_mm_s: tuple[float, ...], schedules: tuple[str, ...], seeds: tuple[int, ...]) -> str:
    return "\n".join(
        [
            "# N3 BO Robustness Study",
            "",
            "This folder contains the tuned-BO robustness study for `n3` over the configured admissible target-speed range.",
            "",
            "## Locked study policy",
            "",
            ...
            "- Tuned BO settings: `EI`, `xi=0.01`, `init_points=2`, `n_iter=30`, `seed_with_theta0=true`.",
            f"- Tuned theta bounds: `{TUNED_THETA_BOUNDS_C}`.",
            "",
            "## Metric definitions",
            "",
            ...
            "## Key outputs",
            "",
            "- `study_summary.csv`: one row per `target x schedule x seed` for tuned `_bov2_` runs only.",
            "- `target_schedule_aggregate.csv`: one row per `target x schedule`.",
            "- `target_aggregate.csv`: optional aggregate across schedules per target.",
            "- `baseline_comparison.csv`: tuned-versus-historical comparison where baseline evidence exists.",
            f"- `run_commands.sh`: full `{len(targets_mm_s) * len(schedules) * len(seeds)}`-run coarse queue.",
            "- `representative_dry_run_command.sh`: one dry-run-config command for config verification.",
            "",
        ]
    )
...
def _scope_note_text(*, targets_mm_s: tuple[float, ...]) -> str:
    return "\n".join(
        [
            ...
            "- This study ingests only `n3` coarse runs whose run name contains `_bov2_` and whose effective config matches the tuned BO signature.",
            "- Historical `n3` runs are used only in `baseline_comparison.csv` as a reference baseline.",
            "- No `n4` run is part of this study.",
            "",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize the tuned-BO robustness study for n3.")
...
def main() -> None:
    ...
    if not bool(bo_config.seed_with_theta0):
        raise ValueError(
            "The active bo.toml has seed_with_theta0=false, but this study is locked to seed_with_theta0=true. "
            "Update bo.toml or the study assumptions first."
        )

    tuned_runs = _scan_runs(
        Path(args.bo_runs_root),
        simulation_config_path=Path(args.simulation_config),
        schedules=schedules,
        loader=_load_tuned_bo_run,
    )
    baseline_runs = _scan_runs(
        Path(args.bo_runs_root),
        simulation_config_path=Path(args.simulation_config),
        schedules=schedules,
        loader=_load_baseline_bo_run,
    )

    tuned_rows = _build_run_rows(
        runs=tuned_runs,
        targets_mm_s=targets_mm_s,
        schedules=schedules,
        seeds=seeds,
    )
    baseline_rows = _build_run_rows(
        runs=baseline_runs,
        targets_mm_s=targets_mm_s,
        schedules=schedules,
        seeds=tuple(sorted({run.random_seed for run in baseline_runs if run.random_seed is not None})),
    )
    target_schedule_aggregate = _group_rows(tuned_rows, group_keys=("target_front_speed_mm_s", "schedule"))
    target_aggregate = _group_rows(tuned_rows, group_keys=("target_front_speed_mm_s",))
    baseline_target_schedule_aggregate = _group_rows(baseline_rows, group_keys=("target_front_speed_mm_s", "schedule"))
    baseline_comparison = _baseline_comparison_rows(
        tuned_aggregates=target_schedule_aggregate,
        baseline_aggregates=baseline_target_schedule_aggregate,
        targets_mm_s=targets_mm_s,
        schedules=schedules,
    )
    ...
    if args.dry_run:
        ...
        print(f"Found tuned BO runs: {len(tuned_runs)}")
        print(f"Found baseline BO runs: {len(baseline_runs)}")
        print(f"Tuned completed rows: {sum(1 for row in tuned_rows if row['status'] == 'completed')}/{len(tuned_rows)}")
        print(f"Baseline completed rows: {sum(1 for row in baseline_rows if row['status'] == 'completed')}/{len(baseline_rows)}")
    ...
    _write_csv(
        output_root / "target_schedule_aggregate.csv",
        (
            "target_front_speed_mm_s",
            "schedule",
            "num_rows",
    ...
    _write_csv(
        output_root / "target_aggregate.csv",
        (
            "target_front_speed_mm_s",
            "num_rows",
            "num_completed",
    ...
    _write_csv(
        ...
        (
            ...
            "tuned_num_completed",
            "tuned_speed_success_fraction",
            "tuned_median_direct_speed_relative_error_pct",
            "tuned_median_infeasible_fraction",
            "tuned_late_best_fraction",
            "tuned_achieved_speed_spread_mm_s",
        ),
        baseline_comparison,
    )
    ...
    _plot_metric_heatmap(
        ...
        title="Tuned n3 BO: speed-success fraction by target and schedule",
    ...
    _plot_metric_heatmap(
        ...
        title="Tuned n3 BO: median infeasible fraction by target and schedule",
    ...
    _plot_metric_heatmap(
        ...
        title="Tuned n3 BO: late-best fraction by target and schedule",
    ...
    _plot_metric_heatmap(
        ...
        title="Tuned n3 BO: median plate MAE by target and schedule",
    ...
    for target_mm_s in targets_mm_s:
        ...
        for schedule in schedules:
            ...
            for seed in seeds:
                ...
                row = matching[0]
                run_name = f"bo_v{_format_speed_tag(target_mm_s)}_n3_{schedule}_{RUN_NAME_TAG}_seed{seed}"
    ...
    _write_shell_script(
        ...
        header_lines=[
            "# Full-range tuned-BO robustness study for n3.",
    ...
    print(f"Tuned n3 BO robustness study written to {output_root.resolve()}")
