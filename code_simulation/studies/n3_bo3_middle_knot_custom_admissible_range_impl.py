...
DEFAULT_OUTPUT_ROOT = velocity_control_diagnostic_dir(3, "bo3_middle_knot_custom_admissible_range")
...
def _target_command(run_name: str, target_mm_s: float, tau_mid: float, seed: int) -> str:
...
def _scan_study_runs(
    ...
    *,
    ...
    candidates = [
        ...
        if path.is_dir() and ("_bov3_" in path.name or "_bov3mk_" in path.name)
    ]
...
def _plot_baseline_vs_custom_envelope(
    ...
    *,
    ...
    ax.set_title("n3 BO3: baseline vs custom middle-knot envelope")
...
def _readme_text(
    *,
    ...
    return "\n".join(
        [
            "# n3 BO3 Middle-Knot Custom Study",
            "",
            "This folder compares the locked historical `n3` BO3 baselines against a new custom middle-knot sweep.",
            "",
            "## Locked study policy",
            "",
            ...
            "- New custom study runs are ingested only when the run name contains `_bov3mk_` and the effective config matches the locked BO3 signature.",
            "- BO3 settings remain locked: `monotone_unit_box`, `feasible_local`, `EI`, `xi=0.01`, `init_points=2`, `n_iter=30`, `seed_with_theta0=true`.",
            ...
            *custom_lines,
            ...
            "## Interpretation rule",
            "",
            "- `n5` is frozen in this phase; the decision question here is whether moving the middle knot in `n3` is already enough to improve the high-speed envelope before revisiting a larger parameterization.",
            "",
        ]
    )
...
def _scope_note_text(*, targets_mm_s: tuple[float, ...], schedule_families: tuple[str, ...]) -> str:
    return "\n".join(
        [
            ...
            f"- Schedule families in this bundle: `{', '.join(schedule_families)}`.",
            "- Historical `n3` BO3 baseline families are shown only for comparison.",
            "- New `custom_tau...` families are the only executable part of this study.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize the n3 BO3 custom middle-knot admissible-range study.")
...
def main() -> None:
    ...
    _plot_metric_heatmap(
        ...
        title="n3 BO3: speed-success fraction by target and schedule family",
    ...
    _plot_metric_heatmap(
        ...
        title="n3 BO3: median direct-speed error by target and schedule family",
    ...
    _plot_metric_heatmap(
        ...
        title="n3 BO3: median plate MAE by target and schedule family",
    ...
    run_commands.append("# Historical baseline families are kept only for comparison.")
    run_commands.append("# Custom middle-knot families remain executable below.")
    run_commands.append("")
    for schedule_family in schedule_families:
        if schedule_family not in CUSTOM_TAU_BY_FAMILY:
            continue
        tau_mid = CUSTOM_TAU_BY_FAMILY[schedule_family]
        run_commands.append(f"# {schedule_family} -> support tau {CUSTOM_SUPPORT_BY_FAMILY[schedule_family]}")
        for target_mm_s in targets_mm_s:
            ...
            for seed in seeds:
                matching = [
                    ...
                    for row in run_rows
                    if _matches_target(float(row["target_front_speed_mm_s"]), target_mm_s)
                    and str(row["schedule_family"]) == schedule_family
                    and int(row["seed"]) == seed
                ]
                row = matching[0]
                run_name = f"bo_v{_format_speed_tag(target_mm_s)}_n3_{schedule_family}_{CUSTOM_RUN_TAG}_seed{seed}"
                if row["status"] == "completed":
                    run_commands.append(f"# Completed {schedule_family}, seed {seed}: {row['selected_run_name']}")
                else:
                    run_commands.extend([_target_command(run_name, target_mm_s, tau_mid, seed), ""])
            run_commands.append("")
        run_commands.append("")
    _write_shell_script(
        output_root / "run_commands.sh",
        header_lines=[
            "# n3 BO3 custom middle-knot admissible-range study.",
            "# Historical baseline runs are never relaunched from this queue.",
        ],
    ...
    print(f"n3 BO3 custom middle-knot study written to {output_root.resolve()}")
