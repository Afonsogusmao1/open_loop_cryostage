...
from code_simulation.optimization.open_loop_workflow_config import build_problem_config
...
DEFAULT_SELECTED_RUN_DIR = (
    results_dir("open_loop_bayesian_optimization")
...
DEFAULT_OUT_DIR = (
    results_dir("open_loop_final_workflow")
...
def _write_workflow_summary(
    *,
    ...
    lines = [
        ...
        "- Scientific objective: historical front-position tracking objective from the previous open-loop evaluator.",
        ...
        "",
        "## Scripts to run",
        "- Inspect active velocity-control BO settings: `python -m code_simulation.optimization.run_velocity_control_bo --dry-run-config`.",
        "- Active velocity-control BO run: `python -m code_simulation.optimization.run_velocity_control_bo --simulation-profile optimization --num-knots 3`.",
        "  Example overrides: `--target-front-speed-mm-s 0.008 --theta0-c 0,-10,-20 --seed 17 --init-points 4 --n-iter 12`.",
        "- Fixed-N BO comparison study: `python -m code_simulation.studies.run_open_loop_fixed_n_bo_study`.",
        "- Reachability/admissibility diagnostics: `python -m code_simulation.verification.run_reachability_diagnostics`.",
        "- Final workflow export: `python -m code_simulation.reporting.export_final_locked_n3_workflow`.",
    ]
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
